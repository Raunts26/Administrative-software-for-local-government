(function(){
  ...
  var Timetable = function() {
    ...
    //console.log(this);
    // HTML elements
    this.hours = ["08.00-09.00", "09.00-10.00", "10.00-11.00", "11.00-12.00", "12.00-13.00", "13.00-14.00", "14.00-15.00", "15.00-16.00", "16.00-17.00"];
    this.days = [" ", "Esmaspäev", "Teisipäev", "Kolmapäev", "Neljapäev", "Reede"];
 ...
 if(this.firstmonth !== this.lastmonth) {
     ...
     blockTime: function() {
        ...
        if(isOkay) {
          ...
          xhttp.open("GET", "../inc/ajax.php?carblock=1&name=" + this.namevalue + "&car=" + this.currenttype + "&date=" + this.blockdate + "&hour=" + this.hour, true);
      ...
      getBlocked: function() {
        ...
        xhttp.open("GET", "../inc/ajax.php?blocked=1&car=" + this.currenttype + "&start=" + start + "&end=" + end, true);
      ...
      createBlocked: function(name, date, hour, amount) {
        ...
        if(this.currenttype === "tablet") {

          for(var i = 0; i < columns.length; i++) {
            if(columns[i].dataset.date === date && columns[i].dataset.hour === hour + "") {

              columns[i].innerHTML = "Jääk: " + (20 - amount);
              columns[i].setAttribute("data-amount",  20 - amount);
              if(columns[i].dataset.amount < 20) {
                columns[i].className = "almost";
              }
              if (columns[i].dataset.amount < 3){
                columns[i].className = "blocked";
              }
      ...
      deleteTime: function(name, dateOrId, hour, pinned) {
        ...
        xhttp.open("GET", "../inc/ajax.php?computer=1&date=" + dateOrId + "&car=" + this.currenttype + "&hour=" + hour + "&delete=1", true);
